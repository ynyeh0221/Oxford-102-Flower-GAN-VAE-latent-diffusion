...
class SimpleUNet(nn.Module):
    def __init__(self, in_channels=3, base_channels=64, time_emb_dim=128):
        ...
        # Learnable residual connection ratio
        self.res_ratio = nn.Parameter(torch.tensor(0.1))  # Initialize with a small value
        ...
        # Decoder
        self.up1 = nn.ConvTranspose2d(base_channels * 4, base_channels * 2, kernel_size=4, stride=2,
                                      padding=1)  # 16->32
    ...
    def forward(self, x, t):
        ...
        # Add residual connection with learnable ratio
        out = out + self.res_ratio * x_input
...
def train_diffusion(diffusion, dataloader, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            generate_samples_grid(diffusion, n_samples=16, save_path=f"./oxford_flowers_image_diffusion/samples_grid_epoch_{epoch}.png", device=device)

            # Create and save an animation of the denoising process
            create_diffusion_animation(diffusion, save_path=f"./oxford_flowers_image_diffusion/diffusion_animation_epoch_{epoch}.gif", num_frames=50, device=device)
